...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeBitholdAmount"))
        settings.setValue("nAnonymizeBitholdAmount", 1000);
    nAnonymizeBitholdAmount = settings.value("nAnonymizeBitholdAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeBitholdAmount"))
        SoftSetArg("-anonymizebitholdamount", settings.value("nAnonymizeBitholdAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeBitholdAmount:
            return QVariant(nAnonymizeBitholdAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeBitholdAmount:
            nAnonymizeBitholdAmount = value.toInt();
            settings.setValue("nAnonymizeBitholdAmount", nAnonymizeBitholdAmount);
            emit AnonymizeBitholdAmountChanged(nAnonymizeBitholdAmount);
            break;
        default:
            break;
        }
