...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeBitholdAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeBitholdAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeBitholdAmount = strAnonymizeBitholdAmount.remove(strAnonymizeBitholdAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeBitholdAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeBitholdAmount*COIN) nMaxToAnonymize = nAnonymizeBitholdAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeBitholdAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeBitholdAmount));
        strAnonymizeBitholdAmount = strAnonymizeBitholdAmount.remove(strAnonymizeBitholdAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeBitholdAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
