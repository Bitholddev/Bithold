...
int nAnonymizeBitholdAmount = 1000;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Bithold
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Bithold
    // Mac: ~/Library/Application Support/Bithold
    // Unix: ~/.bithold
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Bithold";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "bithold.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "bitholdd.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong BITHOLD will not work properly.");
